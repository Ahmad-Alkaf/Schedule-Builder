...
describe('GenerateTableService', () => {
  ...
  beforeEach(() => {
    ...
    v = TestBed.inject(GenerateTableService);
    staticLecs = [
  ...
  it('generateLectures', async () => {
    ...
    let res: SolveLec[] = s.generateScheduleRecursive(staticLecs, solveLecs, []) as SolveLec[];
    ...
    res = s.generateScheduleRecursive(staticLecs, [], []) as SolveLec[];
    ...
    res = s.generateScheduleRecursive([{ name: 'Database', teacher: 'Mohammed', weekDuration: 6, room: 'another' }], [], tables) as SolveLec[];
    for (let r of res)
